...
- **Keys.** The keeper key that CI holds (`GUAsypz1MQ…`) is not the upgrade authority; `admin.ts --set-keeper` rotates it, and `verify-chain.ts` proves a non-keeper upsert is rejected.
